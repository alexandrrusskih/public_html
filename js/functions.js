...
function putImages() {
	if (xmlhttp.readyState == 4) {
		if (xmlhttp.responseText) {
			...
			for (i = 0; i < files.length; i++) {
				if (files[i] != "") {
					if (i%4==0) document.getElementById("container").innerHTML += '<img class="preview1" src=' + files[i] + ' " onCLick="popupImg(this)" />';
					else document.getElementById("container").innerHTML += '<img class="preview" src=' + files[i] + ' " onCLick="popupImg(this)" />';
					j++;
					if (j == 8) j = 0;
				}
			}
